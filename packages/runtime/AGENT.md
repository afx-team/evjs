...
9. **Middleware receives `(ctx, next)`** where `ctx = { fnId, args }` — not a Hono context object
10. **Use `invalidates` on `useMutation()`** for auto cache invalidation — `invalidate()` was removed
