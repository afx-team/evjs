...
const PAGE_ROUTE_TYPES_FILE = "evjs-route-types.d.ts";
const PAGE_ROUTE_TYPES_MARKER =
  "// This file is generated by evjs. Do not edit it directly.";
const STALE_PAGE_ROUTE_TYPES_SKIP_DIRS = new Set([
  ".git",
  ".turbo",
  "dist",
  "node_modules",
...
async function syncPageRouteTypes(
  ...
  const { dir, file } = getPageRouteTypesPath(cwd, routingDir);
...
async function collectGeneratedPageRouteTypeFiles(
  cwd: string,
): Promise<string[]> {
  const files: string[] = [];
  ...
  async function visit(dir: string): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        if (STALE_PAGE_ROUTE_TYPES_SKIP_DIRS.has(entry.name)) continue;
...
async function isGeneratedPageRouteTypesFile(file: string): Promise<boolean> {
  try {
...
function getPageRouteTypesPath(
  cwd: string,
  routingDir: string,
