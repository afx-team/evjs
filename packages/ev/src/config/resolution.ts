...
import { CLIENT_TARGET_MINIMUM } from "../_internal/build/bundler/client-compatibility.js";
import type { BundlerAdapter } from "../_internal/build/bundler/contracts.js";
import {
  assertFrameworkOutputDirectory,
  assertSeparateFrameworkOutputDirectories,
} from "../_internal/build/conventions/output-path-conventions.js";
import { CANONICAL_PAGE_ROUTE_ROOT } from "../_internal/build/conventions/page-route-conventions.js";
...
const PUBLIC_BUNDLER_DEV_CAPABILITY_KEYS = new Set(["clientMiddleware"]);
...
function validateBundlerCapabilities(value: unknown, path: string): void {
  ...
  if (capabilities.dev === undefined) return;
  const dev = assertPlainConfigRecord(
    capabilities.dev,
    `${path}.dev`,
    "a development capabilities object",
  );
  assertKnownConfigKeys(
    dev,
