...
describe("dev", () => {
  ...
  it("commits declaration-only generated changes without a bundler plan update", async () => {
    const cwd = await createProject();
    await writeFile(path.join(cwd, "src/main.ts"), "export {};\n", "utf-8");
    const declarationStatePath = path.join(cwd, "declaration-version.txt");
    const declarationFile = path.join(
      cwd,
      "src/.ev/types/declaration-only-empty-update/database.d.ts",
    );
    const declarationContents = (version: number) =>
      Buffer.from(
        [
          "/* eslint-disable */",
    ...
    await writeFile(declarationStatePath, "1\n", "utf-8");
    const events: string[] = [];
    let updatePlanCalls = 0;
    const plugin: Plugin<Record<string, never>> = {
      name: "declaration-only-empty-update",
      setup(ctx) {
        ctx.addWatchFile(declarationStatePath);
      },
      contributions(ctx) {
        const version = fs.readFileSync(declarationStatePath, "utf-8").trim();
        events.push(`contribution:${version}`);
        const database = ctx.emit.module({
          id: "database",
          scope: { kind: "server" },
          source: "export const version = 1;\n",
          declarationSource: `export declare const version: ${version};\n`,
        });
        ctx.slot("resolve.alias").add({
          id: "database-alias",
          specifier: "evtest:declaration-only",
          replacement: database,
          declaration: {
            exports: [{ kind: "value", name: "version" }],
          },
        });
      },
    };
    ...
    await waitForEvent(events, "bundler.dev");
    await waitForFileContents(declarationFile, declarationContents(1));
    await new Promise((resolve) => setTimeout(resolve, 100));
    await writeFile(declarationStatePath, "2\n", "utf-8");
    await waitForFileContents(declarationFile, declarationContents(2));
    process.emit("SIGINT");
    ...
    await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(
          () => reject(new Error("Declaration-only update timed out.")),
          devUpdateTimeoutMs,
        ),
      ),
    ]);

    expect(events).toContain("contribution:2");
    expect(updatePlanCalls).toBe(0);
  });

  it("restores committed plugin and generated state when a route update fails", async () => {
    const cwd = await createProject();
    const generatedTypeCompanionsPath = path.join(cwd, "src/.ev/types");
    const generatedTypeCompanionPath = path.join(
      generatedTypeCompanionsPath,
      "observe-committed-config",
      "route-count.d.ts",
    );
    const generatedTypeDiscoveryPath = path.join(cwd, "src/evjs-env.d.ts");
    const declarationStatePath = path.join(cwd, "declaration-state.txt");
    await fs.promises.mkdir(path.join(cwd, "src/pages"), { recursive: true });
    await writeFile(declarationStatePath, "enabled\n", "utf-8");
    await writeFile(
      path.join(cwd, "src/pages/page.tsx"),
      "export default function Home() { return null; }",
      "utf-8",
    ...
    const plugin: Plugin<Record<string, never>> = {
      ...
      contributions(ctx) {
        if (fs.readFileSync(declarationStatePath, "utf-8").trim() !== "enabled")
          return;
        const routeCount = ctx.emit.module({
          id: "route-count",
          scope: { kind: "server" },
          source: "export const routeCount = 1;\n",
          declarationSource: "export declare const routeCount: 1;\n",
        });
        ctx.slot("resolve.alias").add({
          id: "route-count-alias",
          specifier: "evtest:route-count",
          replacement: routeCount,
          declaration: {
            exports: [{ kind: "value", name: "routeCount" }],
          },
        });
      },
      setup(ctx) {
      ...
      async dev() {
        ...
        return {
          async updatePlan(_update, options) {
            ...
            const [hasCandidateCompanion, hasCandidateDiscovery] =
              await Promise.all([
                fileExists(generatedTypeCompanionPath),
                fileExists(generatedTypeDiscoveryPath),
              ]);
            events.push(`candidate-companion:${hasCandidateCompanion}`);
            events.push(`candidate-discovery:${hasCandidateDiscovery}`);
    ...
    const initialGeneratedTypeCompanions = await readDirectorySnapshot(
      generatedTypeCompanionsPath,
    );
    const initialGeneratedTypeDiscovery = await fs.promises.readFile(
      generatedTypeDiscoveryPath,
    );
    await writeFile(
      path.join(cwd, "src/pages/about/page.tsx"),
      "export default function About() { return null; }",
    ...
    await writeFile(declarationStatePath, "disabled\n", "utf-8");
    try {
      await Promise.race([
        waitForEvent(events, "update:throw"),
        running.then(() => {
          throw new Error("dev stopped before applying the route update");
        }),
      ]);
    } catch (error) {
      throw new Error(
        `${String(error)}; observed events: ${JSON.stringify(events)}`,
      );
    }
    ...
    await waitForFileContents(
      generatedTypeDiscoveryPath,
      initialGeneratedTypeDiscovery,
    );
    expect(await readDirectorySnapshot(generatedTypeCompanionsPath)).toEqual(
      initialGeneratedTypeCompanions,
    );
    process.emit("SIGINT");
  ...
  it("does not roll back user-owned paths that resemble generated type outputs", async () => {
    const cwd = await createProject();
    const userCompanionPath = path.join(cwd, "src/.ev/types/user.d.ts");
    const userDiscoveryPath = path.join(cwd, "src/evjs-env.d.ts");
    await writeFile(
      path.join(cwd, "src/pages/page.tsx"),
      "export default function Home() { return null; }",
      "utf-8",
    );
    await writeFile(userCompanionPath, "declare const userVersion: 1;\n");
    await writeFile(userDiscoveryPath, "declare const userEnvironment: 1;\n");

    const events: string[] = [];
    const nextCompanion = Buffer.from("declare const userVersion: 2;\n");
    const nextDiscovery = Buffer.from("declare const userEnvironment: 2;\n");
    const bundler: BundlerAdapter<Record<string, never>> = {
      name: "mock",
      capabilities: fullBundlerCapabilities,
      async build() {
      ...
      async dev() {
        ...
        return {
          async updatePlan() {
            await Promise.all([
              fs.promises.writeFile(userCompanionPath, nextCompanion),
              fs.promises.writeFile(userDiscoveryPath, nextDiscovery),
            ]);
            events.push("update:throw");
            throw new Error("mock update failure");
          },
    ...
    await Promise.all([
      waitForFileContents(userCompanionPath, nextCompanion),
      waitForFileContents(userDiscoveryPath, nextDiscovery),
    ]);
    process.emit("SIGINT");
    ...
    expect(events).toEqual(["bundler.dev", "update:throw"]);
  });
  ...
  it("adds and removes static Document aliases during dev updates", async () => {
    ...
    const bundler: BundlerAdapter<Record<string, never>> = {
      ...
      async dev({ plan, callbacks, planGeneration }) {
        const aboutEntry = createPageClientBuildEntryName("about");
        const facts: BundlerBuildFacts = {
          clientEntryAssets: {
            [aboutEntry]: { js: [`${aboutEntry}.js`], css: [] },
        ...
        await callbacks.onBuildFacts(facts, { planGeneration });
