...
| `<routing-dir-parent>/layout.{ts,tsx,js,jsx}` | `routing.layout` | Optional external SPA root layout when one matching file exists. |
| `<routing-dir-parent>/layout/index.{ts,tsx,js,jsx}` | `routing.layout` | Optional external SPA root layout directory form. |
...
| `src/server/routes/**/*.{ts,tsx,js,jsx}` | `server.routing` | Server file routes when `server.routing` is enabled. |
| `src/server/middleware.{ts,tsx,js,jsx}` | `server.conventions.middleware` | Global server middleware when server conventions are enabled. |
...
route directory is `./src/server/routes`. Configure a different directory with
`routing.dir` or `server.routing.dir`.
...
`routing.layout` explicitly. Set `routing.layout: false` to disable external
root layout discovery. MPA mode does not consume framework layouts.
...
`src/server/routes/api/middleware.ts` covers `src/server/routes/api/users.ts`
but not the flat sibling `src/server/routes/api.ts`.
