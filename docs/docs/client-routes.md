...
Navigation can use ordinary anchors or `Link` from `@evjs/ev/client`. Route files
...
keeps scaffolded apps on a single direct framework package. Do not import that
internal helper from application source.
