...
7. The CLI core auto-starts the API server via `@evjs/ev/server/node`.
...
In dev mode, evjs runs the built server bundle through a small Node bootstrap that calls `@evjs/ev/server/node`. For production, deploy the emitted `{ fetch }` handler with the runtime wrapper that matches your host.
