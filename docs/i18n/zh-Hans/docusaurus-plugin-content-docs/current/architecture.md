...
很多框架要求应用在文件树、路由配置、浏览器入口和构建配置中重复同一份信息。evjs 改用少量明确的文件标记：
...
普通组件或辅助模块不会仅仅因为所处位置而变成公共入口。目录只有包含 `page.*` 才会创建页面，只有包含 `api.*` 才会创建 API 路由。这让相关代码可以安全地放在一起，路由规则也更容易理解。

## 页面目录定义职责边界

包含 `page.*` 的目录既决定页面在 URL 中的位置，也是组织该页面代码的自然边界：
...
这种设计鼓励按功能组织代码。团队无需先拼凑分散的路由、元信息和构建文件，就能理解或移动一个页面。多个功能共享的代码仍可放在页面树外。
...
- **SPA** 支持嵌套路由、动态参数、通配路径、布局与客户端导航；可选的
  `routing.basepath` 只为交付后的浏览器路径添加前缀，不改变 Page identity。
...
同一个应用中的不同页面可能有不同交付需求。后台管理页可以在客户端渲染，营销页可以静态生成，账户页可以按请求渲染。
...
插件可以提供带类型的应用选项，并在需要时提供带类型的页面选项。安装插件和配置某个页面是两个独立动作：
...
- 只有明确的 `page.*` 或 `api.*` 文件才会创建公共路由。
- 页面目录决定 URL 位置，并集中组织相关代码和静态配置。
- SPA 与 MPA 是同一棵页面树的两种交付方式。
- 服务端能力可选且可叠加。
- 配置保持在最小合理作用域。
- 插件扩展框架，但不重定义核心概念。
- 部署选择不会反向渗入页面编写方式。
