...
声明框架规划与开发命令行能力；`setup()` 返回需要执行副作用的生命周期钩子。同一项行为
只应放在其中一处。这三部分按职责划分，并不代表三个连续阶段：`configure()` 早于
`setup()`，`emitIR()` 在生成构建输入时执行，`cliShortcuts()` 在开发环境启动或重启时
收集快捷键。描述对象方法不能出现在 `setup()` 返回值中，生命周期钩子也不能直接写在
描述对象上。
