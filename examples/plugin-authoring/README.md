...
- **`emitIR`** — declare deterministic IR records while iterating only enabled, typed Page settings; evjs materializes them later
...
- **`beforeBuild`** — start one framework output/link cycle after the bundler provides fresh facts
- **`afterBuild`** — run logic after that framework output stabilizes
...
2. The enabled, typed Page metadata values in `emitIR`
